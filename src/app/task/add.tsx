...
export default function AddTaskScreen() {
  ...
  // If date was prefilled from calendar, use it, otherwise use today
  const initialDate = params.prefilledDate ? new Date(params.prefilledDate as string) : new Date();
    ...
    const loadCategories = async () => {
      try {
        ...
        if (fetched.length > 0) {
          ...
          setValue('category_id', String(fetched[0].id));
        }
      } catch (error) {
  ...
  const calculateReminderDateTime = (date: Date, time: Date, reminderType: string): Date | null => {
    ...
    // Construct deadline datetime object
  ...
  const onSubmit = async (data: any) => {
    ...
    try {
      ...
      // 4. Schedule push notification if reminder is set
      if (newTask.reminder_datetime) {
        await scheduleTaskReminder(newTask.id, newTask.title, new Date(newTask.reminder_datetime));
      }

      router.back();
    } catch (error: any) {
      setErrorMsg(error.response?.data?.message || 'Gagal menambahkan tugas. Coba lagi.');
    }
  };
